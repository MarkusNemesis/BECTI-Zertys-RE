...
_a_targs= player nearTargets (HUD_MAX_RANGE*2);
...
HUD_T_OBJ=HUD_T_OBJ-[objNull];
{
	...
	if ((player getVariable 'AN_iNet')==CTI_P_SideID) then {
		if ({_x == _t} count (_sl getVariable "CTI_HUD_SHARED") == 0 && ((player getVariable ["CTI_net",-10])  call CTI_CO_FNC_GetSideFromID) == CTI_P_SideJoined && !((_x select 2) == CTI_P_SideJoined)) then {
...
{
	_o=_x;
	if ({_x knowsabout _o >=(missionNamespace getVariable "CTI_EW_HUD_S")} count ((CTI_P_SideJoined) call CTI_CO_FNC_GetSidePlayerGroups) ==0 ) then {_r set [count _r, _x]; ["SERVER", "Server_Hud_Share_Remove",[_o,CTI_P_SideJoined]] call CTI_CO_FNC_NetSend;};
} forEach HUD_T_OBJ;
