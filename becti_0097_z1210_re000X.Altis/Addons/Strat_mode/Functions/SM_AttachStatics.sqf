...
Attach_Static={
  ...
  if (count _NO == 0) exitWith {0 spawn {[["CLIENT",_pl],"Fail_load",[]] call CTI_CO_FNC_NetSend}};
  if !(isNull attachedto (_NO select 0)) exitWith {0 spawn {[["CLIENT",_pl],"Fail_load",[]] call CTI_CO_FNC_NetSend}};
  ...
  (_NO select 0) addEventHandler ["GetOut", {(_this select 2) attachto [(_this select 0),[-3,-1,-0.65]]; detach (_this select 2);}];
...
Action_Statics={
_veh= _this select 0;
_veh  addAction ["<t color='#006400'>Load / Unload Static / Crate</t>", "['SERVER','Request_load',_this]call CTI_CO_FNC_NetSend;", [], 1000, false, true, "", "speed _target <1 && speed _target >-1 && (vehicle _this) == _target && alive _target"];
...
with missionNamespace do {
  ...
  CTI_PVF_Reply_load={
    if (_this select 2) then {
      CTI_P_ChatID commandChat format ["Static Loaded : %1 in %2 ",typeof (_this select 1),_this select 0];
      ...
      CTI_P_ChatID commandChat format ["Static Unloaded : %1 off %2 ",typeof(_this select 1),_this select 0];
      (_this select 0) lockCargo false;
    };

  };
  CTI_PVF_Fail_load={
    hintSilent "No Static Crate found, it must be behind your offroad";
    sleep 2;
    hintSilent "";
  };
};
