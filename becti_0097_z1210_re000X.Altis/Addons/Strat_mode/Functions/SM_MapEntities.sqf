...
_ug=units player;
if ( (player) getVariable "AN_iNet" == _side_id) then {_ug=_ug-[player];};
// player group
{
	...
	if ! ( ( player) getVariable "AN_iNet" == _side_id) exitWith {[_group,[],[]]};
...

{
	...
	if !(isNil {_object getVariable "CTI_Net" }) then {
		...
		if (_side_id == player getvariable ["CTI_Net",-1]) then {
