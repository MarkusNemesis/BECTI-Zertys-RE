...
while {!CTI_GameOver} do
{
	// old values to check for update needed
	{
		...
		missionNamespace setVariable [format ["OLD_AVAILlABLE_%1",_x],_sl getVariable "CTI_AVAILLABLE"];
		missionNamespace setVariable [format ["OLD_ACTIVE_%1",_x],_sl getVariable "CTI_ACTIVE"];
		missionNamespace setVariable [format ["OLD_PRIORITY_%1",_x],_sl getVariable "CTI_PRIORITY"];
	} forEach [east,west];
		...
		if (_x getVariable "cti_town_disactivate") then {
			{
				...
				if (_pr_s == _t && ((_t) getVariable "cti_town_sideID" == _id_s) ) then {
					_pr_s=objNull;
					_sl setVariable ["CTI_PRIORITY",_pr_s,true];
				};
	...
	// Compute available town per side
	{
		...
		if (! isNull (_sl getVariable "CTI_PREVENT")) then {
			_av_s=_av_s-[_sl getVariable  "CTI_PREVENT"];
			_sl setVariable [format ["CTI_PREVENT_%1", _x] , objNull,false];
		};
...

	{
		...
		if (! ((count (_ac_s - _n_ac_s) + count (_n_ac_s - _ac_s))== 0) ) then {_sl setVariable [ "CTI_ACTIVE",_n_ac_s,true];
		};
