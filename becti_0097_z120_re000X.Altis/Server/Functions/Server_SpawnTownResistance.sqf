...
_resistanceSize = round(_value * CTI_TOWNS_RESISTANCE_GROUPS_RATIO / 1000);

//--- Add an extra group for large towns.
if (_value >= 500) then { _resistanceSize = _resistanceSize + 8 };
...
switch (true) do {
	...
	case (_value > 450) : {
		...
		[["GUER_VEHICLES_LIGHT", 1, 27], ["GUER_VEHICLES_MEDIUM", 2, 45], ["GUER_VEHICLES_HEAVY", 1, 42], ["GUER_VEHICLES_AA_LIGHT", 1, 30]]
		];
	};
};
