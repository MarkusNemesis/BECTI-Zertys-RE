...
if (_attempts >= 500) then {
	...
	/*_center=[16244.1,17314.1,-0.0317764];
	_attempts=0;
	_westLocation = _center;
	_eastLocation = _center;
	_range = missionNamespace getVariable "CTI_BASE_STARTUP_PLACEMENT";
	while {(_eastLocation distance _westLocation > _range*1.2 || _eastLocation distance _westLocation < _range) && _attempts <= 500} do {
		_westLocation=[_center, 0, 20000] call CTI_CO_FNC_GetRandomPosition;
		_eastLocation=[_center, 0, 20000] call CTI_CO_FNC_GetRandomPosition;
		diag_log [_eastLocation,_westLocation];
	};*/
