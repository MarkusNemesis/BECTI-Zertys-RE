...
waitUntil {
	...
	//if (_running != _last_running) then {
		...
		_html = "";
		if (_running > -1) then {
			_html = format ["Running: <t color='#F5D363'>%1</t> - Time Remaining: %2 second(s).", ((missionNamespace getVariable format["CTI_%1_UPGRADES_LABELS", CTI_P_SideJoined]) select _running) select 0, call compile format ["RE_VAR_UPGRADECOUNTDOWN_%1", CTI_P_SideJoined]];
		};
