	...
	while { !CTI_GameOver} do
	{
	   ...
	   if (count SM_PATROLS < SM_MAX_PATROLS) then{ //we need to create patrols
	   	...
	   	for "_i" from 1 to _to_create do {
	   		...
	   		_possible_town=[];
	   		{ if ((_x getVariable "cti_town_capture") == CTI_TOWNS_CAPTURE_VALUE_CEIL) then {_possible_town set [count _possible_town,_x]};true} count _resistance_towns;
	    	...
	    	if (count _roads >0 ) then {
		   		...
		   		_veh=[];
		    	{
		    		...
		    		_unitspos=[_unitspos, 50] call CTI_CO_FNC_GetEmptyPosition;
						_v=[_unit, _unitspos, random(360), resistance, false, false, false] call CTI_CO_FNC_CreateVehicle;
...
PATROL_purge_patrol={ //function to remove a patrol group if blocked or dead
	...
	waitUntil {!({alive _x} count (units _group) >0 && !CTI_GameOver && ! _to_delete)};
