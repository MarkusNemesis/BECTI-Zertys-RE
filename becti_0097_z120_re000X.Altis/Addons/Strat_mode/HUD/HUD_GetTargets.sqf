...
{
	if (!((_x select 2) == civilian) && !((_x select 2) == CTI_P_SideJoined) && (group player ) knowsAbout (_x select 4) >=2) then {
...
HUD_SHARED=HUD_SHARED-[objNull];
HUD_T_OBJ=HUD_T_OBJ-[objNull];
{
	_t=(_x select 4);
	_pt=getPosASL (_x select 4);
	_pt set [2,(_pt select 2) + 2];
	if ((player getVariable 'AN_iNet')==CTI_P_SideID) then {
		if ({_x == _t} count HUD_SHARED == 0  &&  /*!terrainIntersectASL [_pt ,positionCameraToWorld [0,0,0]] &&*/ !((_x select 2) == CTI_P_SideJoined) ) then {
			HUD_SHARED = HUD_SHARED + [_t];
			["SERVER", "Server_Hud_Share_Add",[_t,CTI_P_SideJoined]] call CTI_CO_FNC_NetSend;
		};
	};
...
{
	_o=_x;
	if ({_x knowsabout _o >=2} count ((CTI_P_SideJoined) call CTI_CO_FNC_GetSidePlayerGroups) ==0 ) then {_r set [count _r, _x]};
} forEach HUD_T_OBJ;

HUD_T_OBJ=HUD_T_OBJ- _r;
HUD_SHARED=HUD_SHARED - _r ;
...
_return=[];
{
	...
	if (_dis <= HUD_MAX_RANGE && (vehicle _o == _o)  && !((side _o) == civilian) ) then
	{
	  ...
	  _s_icon=0 call {
		...
		/*if (_side == CTI_P_SideJoined && isPlayer _x  ) then {
			_text = name _x;
			if (vehicle _x == _x) then {_icon =""};
		};*/
		...
		_color set [3,_fade];
		if ( {_x == _o} count HUD_SHARED >0 ) then {_shared=true};
