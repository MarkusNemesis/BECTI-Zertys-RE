...
CTI_PREVENT_WEST=objnull;
	...
	CTI_SM_Allow_Capture = "Addons\Strat_mode\Functions\SM_Allow_Capture.sqf";


	CTI_SM_Base_Prot = "Addons\Strat_mode\Functions\SM_Base_Prot.sqf";
	...
	with missionNamespace do {
		...
		CTI_PVF_Server_SM_Priority = {
			missionNamespace setVariable [format ["CTI_PRIORITY_%1", (_this select 0)],(_this select 1)];
			[["CLIENT",(_this select 0)], "Client_SM_priority",(_this select 1)] call CTI_CO_FNC_NetSend;
		};

		CTI_PVF_Server_SM_Disactivate = {
			if (! isNull (missionNamespace getVariable format ["CTI_PRIORITY_%1",_this select 0])) then {
				if ( missionNamespace getVariable format ["CTI_PRIORITY_%1",_this select 0] == _this select 1) then {
					missionNamespace setVariable [format ["CTI_PRIORITY_%1",_this select 0],objNull];
				};
			};
			...
			missionNamespace setVariable [format ["CTI_PREVENT_%1", (_this select 0)] , _this select 1];
		};
		...
		CTI_PVF_Server_Hud_Share_Add= {
			_current=(missionNamespace getVariable format ["CTI_HUD_SHARED_%1",_this select 1]);
			if ( {_x == (_this select 0)}count _current == 0 ) then {
			 _current set [count _current,(_this select 0)];
...
if (CTI_IsClient) then {
	...
	waitUntil {! isNil  {missionNamespace getVariable "CTI_PVF_Client_Base_Zone"} && ! isNil {missionNamespace getVariable "CTI_PVF_Client_Update_Mortars"} && ! isNil  {missionNamespace getVariable "CTI_PVF_Client_Connect"}&& ! isNil {missionNamespace getVariable "CTI_PVF_Client_Availlable_Towns"} && ! isNil{ missionNamespace getVariable "CTI_PVF_Client_SM_priority"} && ! isNil {missionNamespace getVariable "CTI_PVF_SM_message"} };
...
if (CTI_IsServer) then {
		if ( (missionNamespace getVariable 'CTI_SM_STRATEGIC')==1) then {
			0 execVM CTI_SM_Allow_Capture;
		};
		if ( (missionNamespace getVariable 'CTI_SM_BASEP')==1) then {
			 0 execVM CTI_SM_Base_Prot;
		};
...
if (CTI_IsClient) then {
  ...
  0 execVM "Addons\Strat_mode\Functions\SM_3pRestrict.sqf";
	...
	if ( (missionNamespace getVariable 'CTI_SM_HALO')==1) then {
		0 spawn {
			while {!CTI_GameOver&& !  ((side player) == resistance)} do {
				player addAction ["<t color='#ff9900'>HALO jump</t>", "Addons\ATM_airdrop\atm_airdrop.sqf","",100, false, true, "", "vehicle player == player && !CTI_P_PreBuilding && CTI_Base_HaloInRange && [CTI_P_SideJoined, CTI_UPGRADE_HALO, 1] call CTI_CO_FNC_HasUpgrade"];
	...
	// HUD
	if (missionNamespace getVariable "CTI_EW_HUD" == 1 &&  !  ((side player) == resistance)) then {
